...
1. The client receives a job from the server. The job metadata consists of a
path to a 3D model and environment to load.

2. These parameters are passed to the rendering module, along with the rendering
...
3. As part of step 2, the renderer applies a set of "controls," structured scene
transformations.

4. The image (RGB) output of the renderer is sent to the inference module, which
then outputs a prediction. 

5. This prediction, along with any auxiliary information generated by the
...
6. The output of the evaluator is sent back to the client. The client packages
